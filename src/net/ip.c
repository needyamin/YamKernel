...
u16 tcp_checksum(u32 src_ip, u32 dst_ip, const tcp_hdr_t *hdr, usize len) {
    /* TCP pseudo-header */
    struct { u32 src; u32 dst; u8 zero; u8 proto; u16 length; } pseudo;
    pseudo.src    = htonl(src_ip);
    pseudo.dst    = htonl(dst_ip);
    pseudo.zero   = 0;
    pseudo.proto  = IP_PROTO_TCP;
    pseudo.length = htons((u16)len);
    u32 sum = 0;
    const u16 *p = (const u16 *)(const void *)&pseudo;
    for (usize i = 0; i < sizeof(pseudo)/2; i++) sum += p[i];
    p = (const u16 *)(const void *)hdr;
    usize rem = len;
...
u16 udp_checksum(u32 src_ip, u32 dst_ip, const udp_hdr_t *hdr, usize len) {
    struct { u32 src; u32 dst; u8 zero; u8 proto; u16 length; } pseudo;
    pseudo.src = htonl(src_ip); pseudo.dst = htonl(dst_ip);
    pseudo.zero = 0; pseudo.proto = IP_PROTO_UDP; pseudo.length = htons((u16)len);
    u32 sum = 0;
    const u16 *p = (const u16 *)(const void *)&pseudo;
    for (usize i = 0; i < sizeof(pseudo)/2; i++) sum += p[i];
    p = (const u16 *)(const void *)hdr; usize rem = len;
    while (rem > 1) { sum += *p++; rem -= 2; }
    if (rem) sum += *(u8 *)p;
    while (sum >> 16) sum = (sum & 0xFFFF) + (sum >> 16);
    return (u16)~sum;
...
void ip_send(u32 dst_ip, u8 proto, const void *payload, usize payload_len) {
    ...
    if (!arp_lookup(next_hop, dst_mac)) {
        ...
        /* Retry after a brief wait — in practice needs a send queue */
    ...
    ip->id        = htons(g_ip_id++);
    ip->flags_frag = 0x40 << 8; /* Don't fragment */
    ip->ttl       = 64;
    ip->protocol  = proto;
    ip->checksum  = 0;
...
void ip_receive(const ip_hdr_t *hdr, usize total_len) {
    ...
    usize payload_len = total_len - hdr_len;
    ...
    switch (proto) {
        case IP_PROTO_ICMP:
            ...
            break;
        case IP_PROTO_TCP:
            tcp_receive(hdr, (const tcp_hdr_t *)payload, payload + sizeof(tcp_hdr_t),
                        payload_len - sizeof(tcp_hdr_t));
            break;
        case IP_PROTO_UDP:
