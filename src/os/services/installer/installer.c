...
#define INSTALLER_HTTP_PROBE_HOST "ansnew.com"
...
static installer_package_t *find_package(const char *name) {
  ...
  for (usize i = 0; i < sizeof(g_packages) / sizeof(g_packages[0]); i++) {
    if (strcmp(name, g_packages[i].name) == 0)
      return &g_packages[i];
  }
  return NULL;
}
...
static u32 current_caps(void) {
  ...
  if (g_runtime_cache_probe_ok)
    caps |= YAM_INSTALL_CAP_TEMP_STORAGE;
  if (g_runtime_cache_probe_ok)
    caps |= YAM_INSTALL_CAP_PERSISTENT_STORAGE;
  if (g_net_iface.is_up)
    caps |= YAM_INSTALL_CAP_NET_IFACE;
  if (g_net_iface.dhcp_done)
    caps |= YAM_INSTALL_CAP_DHCP;
  if (g_net_iface.dns_server != 0)
    caps |= YAM_INSTALL_CAP_DNS;
  if (g_runtime_tcp_probe_ok)
    caps |= YAM_INSTALL_CAP_TCP_CONNECT;
  if (g_runtime_http_probe_ok)
    caps |= YAM_INSTALL_CAP_HTTP_CLIENT;
  if (cert_store_ready())
    caps |= YAM_INSTALL_CAP_CERT_STORE;
  if (g_runtime_tls_probe_ok)
    caps |= YAM_INSTALL_CAP_HTTPS_TLS;
  return caps;
}

static u32 missing_for_runtime(u32 caps) {
  u32 missing = 0;
  if (!(caps & YAM_INSTALL_CAP_NET_IFACE))
    missing |= YAM_INSTALL_CAP_NET_IFACE;
  if (!(caps & YAM_INSTALL_CAP_DHCP))
    missing |= YAM_INSTALL_CAP_DHCP;
  if (!(caps & YAM_INSTALL_CAP_DNS))
    missing |= YAM_INSTALL_CAP_DNS;
  if (!(caps & YAM_INSTALL_CAP_TCP_CONNECT))
    missing |= YAM_INSTALL_CAP_TCP_CONNECT;
  if (!(caps & YAM_INSTALL_CAP_HTTP_CLIENT))
    missing |= YAM_INSTALL_CAP_HTTP_CLIENT;
  if (!(caps & YAM_INSTALL_CAP_HTTPS_TLS))
    missing |= YAM_INSTALL_CAP_HTTPS_TLS;
  if (!(caps & YAM_INSTALL_CAP_CERT_STORE))
    missing |= YAM_INSTALL_CAP_CERT_STORE;
  if (!(caps & YAM_INSTALL_CAP_PERSISTENT_STORAGE))
    missing |= YAM_INSTALL_CAP_PERSISTENT_STORAGE;
  return missing;
}

static const char *blocker_text(u32 missing) {
  if (missing & YAM_INSTALL_CAP_NET_IFACE)
    return "network interface not ready";
  if (missing & YAM_INSTALL_CAP_DHCP)
    return "DHCP lease not ready";
  if (missing & YAM_INSTALL_CAP_DNS)
    return "DNS resolver not ready";
  if (missing & YAM_INSTALL_CAP_TCP_CONNECT)
    return "TCP client connect path not validated";
  if (missing & YAM_INSTALL_CAP_HTTP_CLIENT)
    return "HTTP client service missing";
  if (missing & YAM_INSTALL_CAP_HTTPS_TLS)
    return "HTTPS/TLS service missing";
  if (missing & YAM_INSTALL_CAP_CERT_STORE)
    return "certificate store missing";
  if (missing & YAM_INSTALL_CAP_PERSISTENT_STORAGE)
    return "persistent package storage missing";
  return "ready";
}

static void probe_runtime_tcp(void) {
  if (g_runtime_tcp_probe_done || !g_net_iface.dhcp_done ||
      !g_net_iface.dns_server)
    return;
...
static void probe_runtime_http(void) {
  if (g_runtime_http_probe_done || !g_runtime_tcp_probe_ok)
    return;
  g_runtime_http_probe_done = true;

  char buf[1024];
  usize len = 0;
  int rc = http_get(INSTALLER_HTTP_PROBE_HOST, INSTALLER_HTTP_PROBE_PATH, buf,
                    sizeof(buf), &len);
  if (rc == 0 && len >= 12 && strncmp(buf, "HTTP/", 5) == 0) {
    g_runtime_http_probe_ok = true;
    char first_line[64];
    usize i = 0;
    for (; i + 1 < sizeof(first_line) && buf[i] && buf[i] != '\r' &&
           buf[i] != '\n';
         i++) {
      first_line[i] = buf[i];
    }
...
void installer_init(void) {
  ...
  kprintf("[INSTALLER] service ready: probe db=%lu net_probe=%s\n",
          sizeof(g_packages) / sizeof(g_packages[0]),
          g_packages[0].official_url);
}

int installer_status(const char *package, yam_installer_status_t *out) {
  if (!g_installer_ready || !out)
    return -1;
