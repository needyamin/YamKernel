...
static char url_buffer[URL_MAX] = "http://example.com/";
static char loaded_url[URL_MAX] = "http://example.com/";
...
static bool starts_with_ci(const char *s, const char *prefix) {
    while (*prefix) {
        if (lower_ascii(*s++) != lower_ascii(*prefix++)) return false;
    }
    return true;
}
...
static const char *find_ci(const char *haystack, const char *needle) {
    ...
    for (const char *p = haystack; *p; p++) {
        if (match_n_ci(p, needle, n)) return p;
    }
    return NULL;
}
...
static void append_text(char *dst, usize cap, usize *pos, const char *src) {
    while (src && *src && *pos + 1 < cap) append_char(dst, cap, pos, *src++);
}
...
static bool tag_is(const char *s, const char *tag) {
    ...
    if (*s == '/') s++;
    usize n = strlen(tag);
    if (!match_n_ci(s, tag, n)) return false;
    char end = s[n];
    return end == '>' || end == ' ' || end == '\t' || end == '/' || end == '\r' || end == '\n';
}

static bool tag_open(const char *s, const char *tag) {
    if (*s != '<' || s[1] == '/') return false;
    usize n = strlen(tag);
    if (!match_n_ci(s + 1, tag, n)) return false;
    char end = s[1 + n];
    return end == '>' || end == ' ' || end == '\t' || end == '/' || end == '\r' || end == '\n';
}

static bool tag_close(const char *s, const char *tag) {
    if (!starts_with_ci(s, "</")) return false;
    usize n = strlen(tag);
...
static bool parse_plain_http_url(const char *url, char *host, usize host_cap, char *path, usize path_cap) {
    if (!starts_with(url, "http://")) return false;
    const char *p = url + 7;
    usize h = 0;
    while (*p && *p != '/' && *p != ':' && h + 1 < host_cap) host[h++] = *p++;
    host[h] = 0;
    if (*p == ':') return false;
    if (*p == '/') text_copy(path, path_cap, p);
    else text_copy(path, path_cap, "/");
    return host[0] != 0;
}
...
static bool extract_attr_value(const char *tag, const char *name, char *out, usize cap) {
    if (!tag || !name || !out || cap == 0) return false;
    out[0] = 0;
    const char *end = tag_end(tag);
    if (!end) return false;
    usize name_len = strlen(name);
    for (const char *p = tag; p < end; p++) {
        if (!match_n_ci(p, name, name_len)) continue;
...
static bool parse_css_color_value(const char *value, u32 *out) {
    ...
    if (starts_with_ci(value, "black")) { *out = 0xFF111827; return true; }
    if (starts_with_ci(value, "white")) { *out = 0xFFFFFFFF; return true; }
    if (starts_with_ci(value, "red")) { *out = 0xFFDC2626; return true; }
    if (starts_with_ci(value, "green")) { *out = 0xFF16A34A; return true; }
    if (starts_with_ci(value, "blue")) { *out = 0xFF2563EB; return true; }
    if (starts_with_ci(value, "gray") || starts_with_ci(value, "grey")) { *out = 0xFF64748B; return true; }
    if (starts_with_ci(value, "orange")) { *out = 0xFFF97316; return true; }
    if (starts_with_ci(value, "purple")) { *out = 0xFF7C3AED; return true; }
    return false;
}

static bool parse_css_decl_color(const char *style, const char *prop, u32 *out) {
...
static void extract_node_style(const char *tag, u32 *fg, u32 *bg, u32 *flags) {
    char style[256];
    u32 c = 0;
    if (!extract_attr_value(tag, "style", style, sizeof(style))) return;
...
static void extract_status_line(const char *raw) {
    ...
    line[i] = 0;
    text_copy(response_meta, sizeof(response_meta), line[0] ? line : "HTTP response received");
}
...
static int http_status_code(const char *raw) {
    ...
    while (*p == ' ') p++;
    int code = 0;
    for (int i = 0; i < 3 && p[i] >= '0' && p[i] <= '9'; i++) code = code * 10 + (p[i] - '0');
    return code;
}

static bool extract_header_value(const char *raw, const char *name, char *out, usize cap) {
    if (!raw || !name || !out || cap == 0) return false;
    out[0] = 0;
    usize name_len = strlen(name);
    const char *p = raw;
    while (*p) {
        ...
        if (line_len > name_len && strncmp(line, name, name_len) == 0 && line[name_len] == ':') {
            const char *v = line + name_len + 1;
            while (*v == ' ' || *v == '\t') v++;
            usize n = 0;
            while (n + 1 < cap && n < line_len && v[n] && v[n] != '\r' && v[n] != '\n') {
                out[n] = v[n];
                n++;
            }
            out[n] = 0;
...
static void extract_title(const char *body) {
    for (usize i = 0; body[i]; i++) {
        if (!tag_open(&body[i], "title")) continue;
        const char *start = tag_end(&body[i]);
        if (!start) break;
        start++;
        const char *end = start;
        while (*end && !tag_close(end, "title")) end++;
        if (end > start) {
            usize n = (usize)(end - start);
            if (n >= sizeof(page_title)) n = sizeof(page_title) - 1;
...
static void render_add_styled(render_node_type_t type, const char *text, u32 fg, u32 bg, u32 flags) {
    if (!text || !*text || render_count >= RENDER_MAX) return;
    while (*text == ' ' || *text == '\n' || *text == '\t') text++;
    if (!*text) return;
...
static void render_flush_line(render_node_type_t type, char *line, usize *line_pos, u32 fg, u32 bg, u32 flags) {
...
static void html_to_text(const char *body) {
    ...
    if (starts_with(body, "HTTP/")) body = body_start(body);

    for (usize i = 0; body[i] && pos + 1 < sizeof(rendered_text); i++) {
        char c = body[i];
        if (skip_script_style) {
            if (tag_close(&body[i], "script") || tag_close(&body[i], "style")) skip_script_style = false;
            continue;
        }
        if (c == '&') {
            char ent = 0;
            if (starts_with(&body[i], "&amp;")) { ent = '&'; i += 4; }
            else if (starts_with(&body[i], "&lt;")) { ent = '<'; i += 3; }
            else if (starts_with(&body[i], "&gt;")) { ent = '>'; i += 3; }
            else if (starts_with(&body[i], "&quot;")) { ent = '"'; i += 5; }
            else if (starts_with(&body[i], "&#39;")) { ent = '\''; i += 4; }
            if (ent) {
        ...
        if (c == '<') {
            if (tag_open(&body[i], "script") || tag_open(&body[i], "style")) {
                render_flush_line(current_type, line, &line_pos, current_fg, current_bg, current_flags);
                ...
                u32 fg = 0, bg = 0, flags = 0;
                extract_node_style(&body[i], &fg, &bg, &flags);
                if (extract_attr_value(&body[i], "value", value, sizeof(value))) render_add_styled(RN_FORM, value, fg, bg, flags);
                else if (extract_attr_value(&body[i], "placeholder", placeholder, sizeof(placeholder))) render_add_styled(RN_FORM, placeholder, fg, bg, flags);
                else render_add_styled(RN_FORM, "Input field", fg, bg, flags);
                ...
                extract_node_style(&body[i], &current_fg, &current_bg, &current_flags);
                       ...
                       tag_open(&body[i], "section") || tag_open(&body[i], "article")) {
                render_flush_line(current_type, line, &line_pos, current_fg, current_bg, current_flags);
                current_type = RN_P;
                current_fg = current_bg = current_flags = 0;
                extract_node_style(&body[i], &current_fg, &current_bg, &current_flags);
            ...
            if (tag_is(&body[i], "br") || tag_is(&body[i], "p") || tag_is(&body[i], "h1") ||
                tag_is(&body[i], "h2") || tag_is(&body[i], "h3") || tag_is(&body[i], "li") ||
                tag_close(&body[i], "div") || tag_close(&body[i], "section") ||
                tag_close(&body[i], "article")) {
                if (pos > 0 && rendered_text[pos - 1] != '\n') append_render_char(&pos, '\n', &line_col);
                if (tag_open(&body[i], "li")) append_text(rendered_text, sizeof(rendered_text), &pos, "- ");
            }
    ...
    render_flush_line(current_type, line, &line_pos, current_fg, current_bg, current_flags);
    if (pos == 0) append_text(rendered_text, sizeof(rendered_text), &pos, "(empty response body)");
    if (render_count == 0) render_add(RN_P, rendered_text);
}

static void history_push(const char *url) {
    if (!url || !*url) return;
    if (history_pos >= 0 && strcmp(history[history_pos], url) == 0) return;
    if (history_count < HISTORY_MAX) {
        history_pos = history_count++;
    } else {
        for (int i = 1; i < HISTORY_MAX; i++) text_copy(history[i - 1], sizeof(history[0]), history[i]);
...
fetch_again:
    ...
    if (starts_with(url_buffer, "https://")) {
        br_state = BR_ERROR;
        text_copy(status_text, sizeof(status_text), "HTTPS needs full encrypted HTTP client. Try http://example.com/");
    ...
    if (!parse_plain_http_url(url_buffer, host, sizeof(host), path, sizeof(path))) {
        br_state = BR_ERROR;
        text_copy(status_text, sizeof(status_text), "Invalid HTTP URL");
        text_copy(rendered_text, sizeof(rendered_text), "Use a URL like http://example.com/");
        return;
    }

    progress = 35;
    text_copy(status_text, sizeof(status_text), "Resolving DNS and connecting...");
    ...
    if (rc < 0) {
        ...
        ksnprintf(rendered_text, sizeof(rendered_text),
                  "Could not load %s\n\nNetwork requirements:\n- e1000 link up\n- DHCP lease\n- DNS server\n- TCP connect\n- plain HTTP server\n\nHTTPS pages need the next encrypted HTTP client layer.",
                  url_buffer);
    ...
    if ((code == 301 || code == 302 || code == 303 || code == 307 || code == 308) &&
        redirect_count < 2 && extract_header_value(raw_page, "Location", location, sizeof(location))) {
    ...
    if (code >= 400) {
        br_state = BR_ERROR;
        ksnprintf(status_text, sizeof(status_text), "Server returned HTTP %d; rendered response body", code);
    } else {
        br_state = BR_DONE;
        ksnprintf(status_text, sizeof(status_text), "Rendered document: %lu bytes from %s", len, host);
    }
    progress = 100;
}

static void draw_button(wl_surface_t *s, i32 x, i32 y, i32 w, const char *label, bool enabled) {
...
static void draw_wrapped_text(wl_surface_t *s, i32 x, i32 *y, const char *text, u32 color, int max_cols) {
    char line[112];
    int col = 0;
    line[0] = 0;
    for (usize i = 0; text[i]; i++) {
        ...
        if (col + 1 < (int)sizeof(line)) line[col++] = c;
    }
    if (col > 0) {
        line[col] = 0;
        wl_draw_text(s, x, *y, line, color, 0);
...
static i32 node_height(render_node_t *n, i32 w) {
    (void)w;
    if (!n) return 0;
    if (n->type == RN_H1) return 66;
    if (n->type == RN_H2) return 32;
    if (n->type == RN_H3) return 22;
    if (n->type == RN_LI) return wrapped_text_height(n->text, 84) + 4;
    if (n->type == RN_LINK) return wrapped_text_height(n->text, 86) + 4;
    if (n->type == RN_PRE) return wrapped_text_height(n->text, 82) + 26;
    if (n->type == RN_IMG) return 86;
    if (n->type == RN_BUTTON) return 42;
    if (n->type == RN_FORM) return 44;
    return wrapped_text_height(n->text, 86) + 8;
}

static void browser_scroll_by(i32 delta) {
    i32 max_scroll = document_height > DOC_VIEW_H ? document_height - DOC_VIEW_H : 0;
    document_scroll += delta;
    if (document_scroll < 0) document_scroll = 0;
    if (document_scroll > max_scroll) document_scroll = max_scroll;
}
...
static void draw_render_tree(wl_surface_t *s, i32 x, i32 y, i32 w) {
    ...
    for (u32 i = 0; i < render_count; i++) {
        ...
        layout_y += h;
        if (!node_visible(cy, h)) continue;
        u32 fg = (n->flags & NODE_STYLE_FG) ? n->fg : 0;
        u32 bg = (n->flags & NODE_STYLE_BG) ? n->bg : 0;
        if (n->type == RN_H1) {
            wl_draw_vgradient(s, x - 14, cy - 12, w - 16, 72, bg ? bg : 0xFFEFF6FF, 0xFFFFFFFF);
            ...
            i32 ty = cy;
            if (bg) wl_draw_rounded_rect(s, x + 12, cy - 5, w - 54, h, 6, bg);
            ...
            wl_draw_rect(s, x, ty - 3, (strlen(n->text) > 86 ? 86 : strlen(n->text)) * 8, 1, 0xFF1D4ED8);
        } else if (n->type == RN_PRE) {
            wl_draw_rounded_rect(s, x - 8, cy - 6, w - 40, h - 8, 6, bg ? bg : 0xFFE2E8F0);
            i32 ty = cy;
            ...
            i32 ty = cy;
            if (bg) wl_draw_rounded_rect(s, x - 8, cy - 6, w - 40, h, 6, bg);
...
static void draw_browser(wl_surface_t *s) {
    ...
    wl_draw_rounded_rect(s, 154, 55, 536, 32, 8, address_focus ? 0xFF1D4ED8 : 0xFF334155);
    ...
    if (br_state == BR_LOADING) wl_draw_rect(s, 0, 96, (BROWSER_W * progress) / 100, 3, 0xFF38BDF8);
    else wl_draw_rect(s, 0, 96, BROWSER_W, 1, 0xFF263244);

    wl_draw_rounded_rect(s, 18, 116, BROWSER_W - 36, BROWSER_H - 164, 9, 0xFFF8FAFC);
    wl_draw_rounded_outline(s, 18, 116, BROWSER_W - 36, BROWSER_H - 164, 9, 0xFFCBD5E1);
    wl_draw_text(s, 38, 136, page_title, 0xFF0F172A, 0);
    wl_draw_rounded_rect(s, 38, 158, 108, 24, 6, br_state == BR_ERROR ? 0xFFFEE2E2 : 0xFFDCFCE7);
    wl_draw_text(s, 52, 163, br_state == BR_ERROR ? "Network" : "Document", br_state == BR_ERROR ? 0xFF991B1B : 0xFF166534, 0);
    wl_draw_text(s, 160, 163, response_meta, br_state == BR_ERROR ? 0xFFB91C1C : 0xFF475569, 0);
    ...
    if (br_state == BR_LOADING) {
        ...
        if (render_count > 0) draw_render_tree(s, 38, 232, BROWSER_W - 76);
        else if (br_state == BR_ERROR) wl_draw_text(s, 38, 232, rendered_text, 0xFF7F1D1D, 0);
        else draw_render_tree(s, 38, 232, BROWSER_W - 76);
    }
    ...
    if (document_height > DOC_VIEW_H) {
        ...
        if (thumb_h < 34) thumb_h = 34;
        i32 max_scroll = document_height - DOC_VIEW_H;
        i32 thumb_y = track_y + ((track_h - thumb_h) * document_scroll) / (max_scroll ? max_scroll : 1);
    ...
    wl_draw_text(s, 16, BROWSER_H - 23, status_text, br_state == BR_ERROR ? 0xFFFFB4B4 : 0xFF94A3B8, 0);
    wl_draw_text(s, BROWSER_W - 342, BROWSER_H - 23, "YamBrowser: styled layout + scroll v0", 0xFF64748B, 0);
}
...
static void browser_go_history(int delta) {
    ...
    if (next < 0 || next >= history_count) return;
    history_pos = next;
...
static void browser_click(void) {
    if (hit(16, 56, 38, 30)) browser_go_history(-1);
    else if (hit(60, 56, 38, 30)) browser_go_history(1);
    else if (hit(104, 56, 38, 30)) browser_fetch_current();
    else if (hit(154, 55, 536, 32)) address_focus = true;
    else if (hit(702, 55, 94, 32)) browser_fetch_current();
    else address_focus = false;
}
...
static void browser_key(u16 sc) {
    ...
    if (c == '\n') {
        ...
        if (address_focus) browser_go_history(-1);
        else browser_scroll_by(-42);
    } else if (sc == 0x50) {
        if (address_focus) browser_go_history(1);
        else browser_scroll_by(42);
    } else if (sc == 0x49) {
...
void wl_browser_task(void *arg) {
    ...
    wl_surface_t *s = wl_surface_create("YamBrowser", 150, 80, BROWSER_W, BROWSER_H, sched_current()->id);
    if (!s) return;

    text_copy(rendered_text, sizeof(rendered_text),
              "Type a plain HTTP URL and press Enter.\n\nTry:\n- http://example.com/\n- http://neverssl.com/\n\nThis browser now has a real plain-HTTP network path and a static DOM paint layer. HTTPS page loading, JavaScript, full CSS layout, images, and Mozilla-class rendering are still engine work.");
    render_count = 0;
    render_add(RN_H1, "YamBrowser");
    render_add(RN_P, "Plain HTTP pages are fetched by the YamOS kernel network stack, parsed into static document nodes, and painted by the compositor.");
    render_add(RN_LI, "Works now: DNS, TCP, HTTP/1.0, redirects, headings, paragraphs, links, forms, buttons, image placeholders.");
    render_add(RN_LI, "Still required for Firefox-class browsing: encrypted HTTPS stream, certificate validation, HTML5 tree builder, CSS cascade/layout, JavaScript, images, storage, cache, cookies, sandboxing.");
    draw_browser(s);
    wl_surface_commit(s);
    ...
    while (s->state == WL_SURFACE_ACTIVE && s->id == my_id) {
        ...
        while (wl_surface_pop_event(s, &ev)) {
            ...
            else if (ev.type == EV_REL && ev.code == REL_WHEEL) browser_scroll_by(ev.value < 0 ? 72 : -72);
            else if (ev.type == EV_KEY) {
                if (ev.code == 0x2A || ev.code == 0x36) {
                    shift_held = ev.value == KEY_PRESSED;
                } else if (ev.value == KEY_PRESSED) {
                    if (ev.code >= 0x110) browser_click();
                    else browser_key(ev.code);
                }
            }
        }
        draw_browser(s);
